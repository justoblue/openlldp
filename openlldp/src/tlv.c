...
extern LLDP_NEIGHBOR_SHELFID* g_lldp_neighbor_shelfid;
extern uint8_t g_LLDP_TYPE;
extern uint8_t LLDP_TYPE_MCN;
extern uint8_t LLDP_TYPE_SHELFID;
extern uint32_t gMgtAddressIpv4;
extern uint8_t gMgtAddressIpv6[LLDP_IPV6_SIZE];
extern uint8_t glldpIpType ;
  ...
  char *title  = NULL;
    ...
    switch(tlv->type)
    {
        case CHASSIS_ID_TLV:
            {
              ...
              if ((g_LLDP_TYPE == LLDP_TYPE_SHELFID) && debug_hex_strcpy(g_lldp_neighbor_shelfid->chassis_id, chassis_id, (tlv->length - 1), CHASSISID_SIZE))
              {
		...
		//	strncat(result, "Port ID: ", 2048);
              ...
              if ((g_LLDP_TYPE == LLDP_TYPE_SHELFID) && debug_memcpy(g_lldp_neighbor_shelfid->port_id, port_id, (tlv->length - 1), PORTID_SIZE))
              {
                g_lldp_neighbor_shelfid->isChanged = 1;
              }
        ...
        if ((g_LLDP_TYPE == LLDP_TYPE_SHELFID) && htons(*ttl) == 0)
        {
          g_lldp_neighbor_shelfid->isChanged = 1;
          memset(g_lldp_neighbor_shelfid->chassis_id, 0x0, CHASSISID_SIZE);
          memset(g_lldp_neighbor_shelfid->port_id, 0x0, PORTID_SIZE);
          memset(g_lldp_neighbor_shelfid->system_name, 0x0, SYSNAME_SIZE);
        }
        ...
        case SYSTEM_NAME_TLV:
	      ...
	      char *cstr = tlv_info_string_to_cstr(tlv);
          if ((g_LLDP_TYPE == LLDP_TYPE_SHELFID) && debug_memcpy(g_lldp_neighbor_shelfid->system_name, (uint8_t*)tlv->info_string, (tlv->length), SYSNAME_SIZE))
          {
            g_lldp_neighbor_shelfid->isChanged = 1;
          }
...
void ipv6_to_str_unexpanded(char* str, const uint8_t * addr) {
    ...
    case IANA_IP6:
      {
        if(addr_len == 17) {
          tmp = calloc(1, 64);
              ipv6_to_str_unexpanded((char *)tmp, (const uint8_t *)addr);
              strncat(result, (char *)tmp, 2048);
              free(tmp);
        } else {
          debug_printf(DEBUG_NORMAL, "Invalid IPv6 address length: %d\n", addr_len);
          debug_hex_strcat((uint8_t *)result, addr, addr_len - 1);
        }
      }break;
    default:
...
char *decode_tlv_system_capabilities( uint16_t system_capabilities, uint16_t enabled_capabilities)
{
    ...
    char *capability_string = malloc(512);
    ...
    {
      memset(capability_string, 0x0, 512);
	    if(strlen(result) >= (2048-512))
        {
            debug_printf(DEBUG_NORMAL, "decode_tlv_system_capabilities result over length\n");
            break;
        }
        if((system_capabilities & capability))
        {
	      ...
	      strncat(result, capability_string, 512);
            }
            else
            {
...
struct lldp_tlv *create_mcn_chassis_id_tlv(struct lldp_port *lldp_port) {
    ...
    // We need to start copying at the 2nd byte, so we use [1] here...
    // This reads "memory copy to the destination at the address of tlv->info_string[1] with the source my_mac for 6 bytes" (the size of a MAC address)
    memcpy(&tlv->info_string[1], &lldp_port->chassis_id[0], 6);
...
struct lldp_tlv *create_mcn_management_address_tlv(struct lldp_port *lldp_port)
{
    if (glldpIpType == 0)
    {
        return NULL;
...
#define OBJ_IDENTIFIER 0
    ...
    uint8_t ipLength = (glldpIpType == 1)? IPV4_LEN: IPV6_LEN;
    tlv->length = MGMT_ADDR_STR_LEN + MGMT_ADDR_SUBTYPE + ipLength + IF_NUM_SUBTYPE + IF_NUM + OID + OBJ_IDENTIFIER ;
    ...
    tlv->info_string[0] = 1+ ipLength;

    // 1 for IPv4 as per http://www.iana.org/assignments/address-family-numbers
    // 2 for IPv6
    tlv->info_string[1] = glldpIpType;

    // Copy in our IP
    if (glldpIpType == 1)
    {
        memcpy((void *)&tlv->info_string[2], (void *)&gMgtAddressIpv4, IPV4_LEN);

    }
    else
    {
        memcpy((void *)&tlv->info_string[2], (void *)gMgtAddressIpv6, IPV6_LEN);
    }
    // Interface numbering subtype... system port number in our case.
    tlv->info_string[2+ipLength] = 3;

    // Interface number... 4 bytes long, or uint32_t
    memcpy(&tlv->info_string[2+ipLength+1], &lldp_port->if_index, 4);

    debug_printf(DEBUG_NORMAL, "Would stuff interface #: %d\n", if_index);

    // OID - 0 for us
    tlv->info_string[2+ipLength+1+4] = 0;
