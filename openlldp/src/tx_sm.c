...
extern uint8_t g_dstMac[6];
extern uint8_t g_use_dstMac;
extern uint16_t g_MsgTxHold;
extern uint16_t g_MsgTxInterval;
...
void mibConstrInfoLLDPDU(struct lldp_port *lldp_port)
{
    ...
    {
        tx_hdr.dst[0] = g_dstMac[0];
        tx_hdr.dst[1] = g_dstMac[1];
        tx_hdr.dst[2] = g_dstMac[2];
        tx_hdr.dst[3] = g_dstMac[3];
        tx_hdr.dst[4] = g_dstMac[4];
        tx_hdr.dst[5] = g_dstMac[5];
    }
#ifndef WIN32
...
void mibConstrInfoOSCLLDPDU(struct lldp_port *lldp_port)
{
    // This code will only work for Linux!
    struct eth_infn_hdr tx_hdr;
    struct lldp_tlv_list *tlv_list = NULL;
	struct lldp_flat_tlv *tlv      = NULL;
	struct lldp_tlv_list *tmp      = 0;
	uint32_t frame_offset          = 0;
    ...
    // This should be replaced with OS-specific events if possible...
#ifndef __TARGET__
    refreshInterfaceData(lldp_ports);
#endif
    ...
    tx_hdr.src[5] = lldp_port->source_mac[5];
#else
    tx_hdr.src[0] = lldp_ports->source_mac[0];
    tx_hdr.src[1] = lldp_ports->source_mac[1];
    tx_hdr.src[2] = lldp_ports->source_mac[2];
    tx_hdr.src[3] = lldp_ports->source_mac[3];
    tx_hdr.src[4] = lldp_ports->source_mac[4];
    tx_hdr.src[5] = lldp_ports->source_mac[5];
#endif

    tx_hdr.infntype = htons(0x99a1);

#ifdef __TARGET__
    tx_hdr.infntag[0] = 0x4e;
#else
    tx_hdr.infntag[0] = 0x0e;//test
#endif

    tx_hdr.infntag[1] = lldp_port->shelf_id;
    tx_hdr.infntag[2] = lldp_port->slot_id;
    tx_hdr.infntag[3] = lldp_port->port_id;
    tx_hdr.ethertype = htons(0x88cc);
    ...
    if(lldp_port->tx.frame != NULL)
    {
        memcpy(&lldp_port->tx.frame[frame_offset], &tx_hdr, sizeof(struct eth_infn_hdr));
    }

    frame_offset += sizeof(struct eth_infn_hdr);

    // This TLV *MUST* be first.
    add_tlv(create_mcn_chassis_id_tlv(lldp_port), &tlv_list);
    ...
    if (glldpIpType != 0)
    {
        add_tlv(create_mcn_management_address_tlv(lldp_port), &tlv_list);
    }
    //add LLDP-MED location identification
    if (lci.location_data_format != -1)
      add_tlv (create_lldpmed_location_identification_tlv (lldp_port),
...
void mibConstrShutdownOSCLLDPDU(struct lldp_port *lldp_port)
{
    // This code will only work for Linux!
    struct eth_infn_hdr tx_hdr;
    struct lldp_tlv_list *tlv_list = NULL;
...
uint8_t txFrame(struct lldp_port *lldp_port)
{
    ...
    if (lldp_port->tx.statistics.statsFramesOutTotal == 0xFFFFFFFF)
    {
...
uint8_t txOscFrame(struct lldp_port *lldp_osc_port, struct lldp_port *lldp_port)
{
    ...
    // lldp_port contains all of the necessary information to do platform-specific
    // transmits.
    lldp_osc_write(lldp_osc_port, lldp_port);

    if(lldp_osc_port->tx.frame != NULL)
      {
	// Should only need to memset 0x0 to sendsize, because we shouldn't have filled it more than that. ;)
	memset(&lldp_osc_port->tx.frame[0], 0x0, lldp_osc_port->tx.sendsize);
      }
    // Update the statsFramesOutTotal counter...
    lldp_osc_port->tx.statistics.statsFramesOutTotal++;
    if (lldp_osc_port->tx.statistics.statsFramesOutTotal == 0xFFFFFFFF)
    {
        lldp_osc_port->tx.statistics.statsFramesOutTotal = 0;
    }
    return 0;
}
...
uint8_t txInitializeLLDP(struct lldp_port *lldp_port)
{
    ...
    {
        lldp_port->tx.timers.msgTxHold     = g_MsgTxHold;  // Recommended minimum by 802.1AB 10.5.3.3
        lldp_port->tx.timers.msgTxInterval = g_MsgTxInterval; // Recommended minimum by 802.1AB 10.5.3.3
    }
    else
    {
