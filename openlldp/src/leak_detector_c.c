...
void add(MEM_INFO alloc_info)
{
     ...
     //printf("leak_detector_c add %p\n", mem_leak_info->mem_info.address);
...
void erase(unsigned pos)
{
     ...
     MEM_LEAK * alloc_info, * temp;
      ...
      if(pos == 0)
      {
          ...
          {
              ptr_next = ptr_start;
              //printf("erase 1st index=%d, temp == ptr_next\n",index);
          }
      ...
      else 
      {
              ...
              alloc_info = alloc_info->next, ++index)
          {
              if(pos == index + 1)
              {
                  ...
                  {
                      ptr_next = alloc_info;
                      //printf("erase index=%d, temp == ptr_next\n",index);
                  }
                  free(temp);
                  break;
              }
  ...
  {
      //printf("leak_detector_c xfree %p\n", mem_ref);
...
*/
 ...
 void remove_mem_info (void * mem_ref)
 {
    ...
    for(index = 0; leak_info != NULL; ++index, leak_info = leak_info->next)
    {
        ...
        {
             //printf("leak_detector_c remove_mem_info %p\n", leak_info->mem_info.address);
             erase ( index );
             break;
        }
    }
