...
ssize_t lldp_osc_write(struct lldp_port *lldp_osc_port, struct lldp_port *lldp_port) {
  ...
  // Write the frame to the wire
  return sendto(lldp_port->socket, lldp_osc_port->tx.frame, lldp_osc_port->tx.sendsize, 0, NULL, 0);  
}
...
int socketInitializeLLDP(struct lldp_port *lldp_port,uint32_t protocolType)
{
    ...
    lldp_port->socket = socket(PF_PACKET, SOCK_RAW, htons(protocolType));
    ...
    sll->sll_protocol = htons(protocolType);
    ...
    int socketFlags = fcntl(lldp_port->socket, F_GETFL, 0);
    if (-1 == fcntl(lldp_port->socket, F_SETFL, socketFlags|O_NONBLOCK))
    {
        debug_printf(DEBUG_NORMAL, "fcntl socket error!\n");
    }
#if 0
    ...
    lldp_port->rx.frame = calloc(1, lldp_port->mtu);
    ...
    if (protocolType == 0x88CC)
    {
        lldp_port->tx.frame = calloc(1, (lldp_port->mtu));
