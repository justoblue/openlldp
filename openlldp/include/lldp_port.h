...
#define MD5_MAX_KEY_LENGTH 16
#define ORLLDP_MAX_NEIGHBORS_PERPORT         1

/* Frame Type Classification - Used diring Rx LLDPDU Processing */
enum
{
    ORLLDP_RX_FRAME_NEW = 1,
    ORLLDP_RX_FRAME_UPDATE = 2,
    ORLLDP_RX_FRAME_REFRESH = 3,
    ORLLDP_RX_FRAME_SHUTDOWN = 4,
    ORLLDP_RX_FRAME_INVALID = 0xFF
};
...
struct lldp_rx_port {
    uint8_t *frame;
    ssize_t recvsize;
    uint8_t pduMsgDigest[MD5_MAX_KEY_LENGTH];
    uint8_t rxFrameType;
    uint8_t state;
    uint8_t badFrame;
    uint8_t rcvFrame;
    ...
    uint8_t remoteIndexArray[ORLLDP_MAX_NEIGHBORS_PERPORT];
